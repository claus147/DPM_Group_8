	...
	public static void main(String[] args) {
		...
		if (buttonChoice == Button.ID_LEFT) {					//for the auto selector
			if (usR.getDistance() < 80||usLeft.getDistance() < 80){							//80 is the number specified to be the "end" of wall
				usloc = new USLocalizer(odo, nav, usLeft, usR, USLocalizer.LocalizationType.FALLING_EDGE);
			} else {
				usloc = new USLocalizer(odo, nav, usLeft, usR, USLocalizer.LocalizationType.RISING_EDGE);
			}
			...
			if (buttonChoice == Button.ID_LEFT) {
				usloc = new USLocalizer(odo, nav, usR, usLeft, USLocalizer.LocalizationType.RISING_EDGE);		//set to rising edge
			} else {
				usloc = new USLocalizer(odo, nav, usR, usLeft, USLocalizer.LocalizationType.FALLING_EDGE);		//set to falling edge
			}
		}
